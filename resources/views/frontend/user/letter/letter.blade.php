...
<div class="">
    <div class="box">
        ...
        <div class="para">
            <p>Perticipants of the different organization under the Mnistry of Agriculture joined the training course entitle "{{ $letter->course->course_name }}" held
        ...
        <table style="width:100%;padding-bottom:30px">
            ...
            <tbody>
            @foreach($realese as $l)
                <tr>
                    <td style="text-align:center">{{ $loop->index+1 }}</td>
                    <td >{{ $l->name_en }}</td>
                    <td >{{ $l->designation }}</td>
                    <td >{{ $l->working_station }},{!! "&nbsp;" !!}{{ $l->c_upozila == 0 ? '' : $l->proupozila->upozila_name }}{{ $l->c_upozila == 0 ? '' : ',' }}{!! "&nbsp;" !!}{{ $l->prodistrict->district_name }}</td>
    ...
    <div style="clear:both;margin:0;padding:0;">
        ...
        <div style="width:70%; display: inline-block; float:left;">
            ...
            <ol>
                @foreach($orgs as $one)
                    <li>{{ $one->controlling_officer }},{!! "&nbsp;" !!}{{ $one->organization_name }},{!! "&nbsp;" !!}{{ $one->c_upozila==0 ? '' : $one->proupozila->upozila_name }}{{ !empty($one->proupozila->upozila_name) ? ',' : '' }}{!! "&nbsp;" !!}{{ $one->prodistrict->district_name }}</li>
                @endforeach
            </ol>
